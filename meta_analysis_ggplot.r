...
test_heterogeneity(data = dat, overall_res, mods = c("Data source", "Input data source", "Forecast horizon", "Training and testing approach", "First author, year"))
# test_heterogeneity_multivariate(data = dat, overall_res, mods = c("Data source", "Input data source"), interaction = TRUE)
...
for (sg in rev(names(res_dict))) {
    ...
    new_entries <- data.frame(
        ...
        seizures = dat_subgroup$"Total # seizures",
...
p_labels <-
    ...
    draw_labels(pos = 2, key = entries$size, hjust = 0.5, label = entries$label) +
    draw_labels(pos = 2.5, key = entries$seizures, hjust = 0.5, label = entries$label) +
...
layout <- c(
    ...
    area(t = 0, l = 7, b = 30, r = 9), # starts 1 unit right of the left plot (l=4, whereas left plot is r=3), goes to the bottom of the page (30 units), and 6 units further over from the left plot (r=9 whereas left plot is r=3)
